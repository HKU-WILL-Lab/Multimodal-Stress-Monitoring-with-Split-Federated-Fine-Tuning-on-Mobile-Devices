...
class SuffixTrainer:
    ...
    def __init__(
        ...
        *,
        ...
        if checkpoint_interval_steps <= 0:
            raise InvalidRequest("checkpoint_interval_steps must be positive")
    ...
    def train_step(
        ...
        with self._lock:
            ...
            selected = mask if loss_mask is None else loss_mask.to(self.device)
            loss, accuracy = shifted_token_objective(logits, ids, mask, selected)
            loss.backward()
            ...
            self.optimizer.step()
            self._server_step += 1
            if self._server_step % self.checkpoint_interval_steps == 0:
