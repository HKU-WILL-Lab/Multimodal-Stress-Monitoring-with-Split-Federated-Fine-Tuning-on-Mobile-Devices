...
def shifted_token_objective(
    ...
    if loss_mask.dtype != torch.int64 or loss_mask.ndim != 2:
        raise InvalidRequest("loss mask must be rank-two int64")
    if (
        logits.shape[:2] != token_ids.shape
        or token_ids.shape != attention_mask.shape
        or token_ids.shape != loss_mask.shape
    ):
        raise InvalidRequest("logits, token IDs, and mask batch/sequence shapes differ")
    if logits.shape[1] < 2:
    ...
    if not torch.logical_or(loss_mask == 0, loss_mask == 1).all():
        raise InvalidRequest("loss mask values must be zero or one")
    if (loss_mask > attention_mask).any():
        raise InvalidRequest("loss mask cannot select a padded token")
