...
std::string client_usage(const char* program) {
    ...
    out << "Usage: " << program << " [options]\n"
        ...
        << "  --model-dir PATH            External Gemma 3 270M asset directory\n"
        << "  --dataset PATH              Raw UTF-8 WikiText-style file\n"
...
ClientOptions parse_client_options(int argc, char** argv) {
    ...
    if (options.protocol_version == 0 || options.cut_layer == 0 ||
        options.local_steps == 0 || options.batch_size == 0 ||
