...
& adb @AdbPrefix reverse "tcp:$SuffixPort" "tcp:$SuffixPort"
if ($LASTEXITCODE -ne 0) { throw 'Could not reverse suffix port' }
& adb @AdbPrefix reverse "tcp:$CoordinatorPort" "tcp:$CoordinatorPort"
if ($LASTEXITCODE -ne 0) { throw 'Could not reverse coordinator port' }

$Command = "cd '$DeviceRoot' && ./sfl_android_client" +
    " --config '$DeviceRoot/smoke_single_phone.json'" +
    " --client-id '$ClientId'" +
    " --model-dir '$DeviceRoot/model'" +
    " --dataset '$DeviceRoot/data/wikitext.txt'" +
